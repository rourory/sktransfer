...
// Убираем "use client" - теперь это Серверный Компонент!
export async function Footer() {
  const currentYear = new Date().getFullYear();

  // 1. Получаем текущий язык из Cookies
  // Замените "NEXT_LOCALE" на то имя куки, которое использует ваш переключатель языков
  // Если у вас пока нет куки, по умолчанию будет "ru"
  const cookieStore = await cookies();
  const localeValue = cookieStore.get("NEXT_LOCALE")?.value || cookieStore.get("locale")?.value || "ru";
  
  // Строгая типизация локали
  const locale = (localeValue === "en" || localeValue === "zh") ? localeValue : "ru";
  const t = translations[locale];

  // 2. Получаем категории из базы прямо здесь!
      ...
      <div className="container mx-auto px-4 sm:px-6 py-12 sm:py-16 relative z-10">
        <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-8 sm:gap-10 lg:gap-12 mb-8 sm:mb-12">
          ...
          <div className="lg:col-span-2">
            <Link href="/" className="inline-block mb-4 sm:mb-6 group relative">
              ...
              >
                ...
                <text
                  x="120" y="50" fontFamily="'Times New Roman', serif" fontSize="52" fontWeight="bold"
                  fill="url(#whiteGradientFooter)" filter="url(#glowFooter)" textAnchor="middle"
                >
                  SK
                </text>
                <text
                  x="120" y="75" fontFamily="'Times New Roman', serif" fontSize="16" fontWeight="normal"
                  fill="url(#whiteGradientFooter)" letterSpacing="4" textAnchor="middle"
                >
                  TRANSFER
                </text>
              </svg>
            ...
            <div className="flex gap-2 sm:gap-3">
              <a href="https://t.me/+375291228484" target="_blank" rel="noopener noreferrer" className="p-2 sm:p-2.5 bg-white/10 hover:bg-white/20 border border-white/20 hover:border-white/40 rounded-lg transition-all shadow-lg shadow-white/5 hover:shadow-xl hover:shadow-white/10 hover:scale-110">
                <FontAwesomeIcon icon={faTelegram} className="text-white text-lg sm:text-xl drop-shadow-md" />
              </a>
              <a href="https://wa.me/375291228484" target="_blank" rel="noopener noreferrer" className="p-2 sm:p-2.5 bg-white/10 hover:bg-white/20 border border-white/20 hover:border-white/40 rounded-lg transition-all shadow-lg shadow-white/5 hover:shadow-xl hover:shadow-white/10 hover:scale-110">
                <FontAwesomeIcon icon={faWhatsapp} className="text-white text-lg sm:text-xl drop-shadow-md" />
              </a>
              <a href="viber://chat?number=375291228484" className="p-2 sm:p-2.5 bg-white/10 hover:bg-white/20 border border-white/20 hover:border-white/40 rounded-lg transition-all shadow-lg shadow-white/5 hover:shadow-xl hover:shadow-white/10 hover:scale-110">
          ...
          <div>
            ...
            <ul className="space-y-2 sm:space-y-3">
              {[
                ...
                <li key={link.href}>
                  <Link href={link.href} className="text-gray-300 hover:text-white transition-all flex items-center gap-2 group hover:translate-x-1">
          ...
          <div>
            ...
            <ul className="space-y-2 sm:space-y-3">
              {categories.map((cat) => {
                const name = locale === "ru" ? cat.nameRu : locale === "en" ? cat.nameEn : cat.nameZh;
                
                return (
                  <li key={cat.slug}>
                    <Link href={`/blog/${cat.slug}`} className="text-gray-300 hover:text-white transition-all flex items-center gap-2 group hover:translate-x-1">
              ...
              <li className="pt-2">
                <Link href="/blog" className="text-[var(--gold)] font-medium hover:text-yellow-400 transition-all flex items-center gap-2 group hover:translate-x-1">
                  <span className="w-0 group-hover:w-2 h-0.5 bg-[var(--gold)] transition-all" />
                  {locale === "ru" ? "Все статьи →" : locale === "en" ? "All articles →" : "所有文章 →"}
                </Link>
              </li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3 sm:space-y-4">
              <li>
                <a href="tel:+375291228484" className="flex items-start gap-3 text-gray-300 hover:text-white transition-all group">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-400">{locale === "ru" ? "Беларусь" : locale === "en" ? "Belarus" : "白俄罗斯"}</div>
                  </div>
                </a>
              </li>
              <li>
                <a href="mailto:info@sktransfer.by" className="flex items-start gap-3 text-gray-300 hover:text-white transition-all group">
                  <div className="p-2 bg-white/10 border border-white/20 rounded-lg group-hover:bg-white/20 group-hover:border-white/40 transition-all shadow-lg shadow-white/5">
        ...
        <div className="border-t border-white/15 pt-6 sm:pt-8 mt-6 sm:mt-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-xs sm:text-sm text-gray-300">
            <div className="text-center md:text-left">
              © {currentYear} SKTransfer.by. {locale === "ru" ? "Все права защищены" : locale === "en" ? "All rights reserved" : "版权所有"}
            </div>
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 md:gap-6 items-center">
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-6 items-center text-center">
                <Link href="/privacy" className="hover:text-white transition-colors">{locale === "ru" ? "Политика конфиденциальности" : locale === "en" ? "Privacy Policy" : "隐私政策"}</Link>
                <Link href="/terms" className="hover:text-white transition-colors">{locale === "ru" ? "Условия использования" : locale === "en" ? "Terms of Use" : "使用条款"}</Link>
              </div>
              <a href="https://netnext.site/" target="_blank" rel="noopener noreferrer" className="text-xs hover:text-white transition-colors text-center">
